...
export function ProdutoTable({ produtos, onEditar, onRemover }: ProdutoTableProps) {
  return (
        ...
        <tbody>
          {produtos.map((produto) => (
            <tr key={produto.id} className="border-b border-grid last:border-0">
              ...
              <td className="px-4 py-3 tabular-nums text-ink-secondary">{formatarMoeda(produto.custoUnitario)}</td>
